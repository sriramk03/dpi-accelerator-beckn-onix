...
class MockInstallerStateService {
  private state = new BehaviorSubject<{ gcpConfiguration: GcpConfiguration | null }>({
...
describe('StepGcpConnectionComponent', () => {
  ...
  describe('Initialization and API Fetching', () => {
    it('should initialize the form and fetch projects and regions on init', fakeAsync(() => {
      const fetchProjectsSpy = spyOn(apiService, 'getGcpProjectNames').and.callThrough();
      const fetchRegionsSpy = spyOn(apiService, 'getGcpRegions').and.callThrough();
    ...
    it('should initialize form with values from installer state', fakeAsync(() => {
        const initialState = { gcpConfiguration: { projectId: 'gcp-project-1', region: 'us-east1' } };
        installerStateService.setInitialState(initialState);

        fixture.detectChanges();
        tick();

        expect(component.gcpConnectionForm.value).toEqual(initialState.gcpConfiguration);
    }));
  });
  ...
  describe('Form Interaction and Filtering', () => {
    ...
    it('should filter projects based on search input', fakeAsync(() => {
      ...
      tick(300);
      expect(component.filteredGcpProjects.value).toEqual(['gcp-project-1', 'gcp-project-2']);

      component.projectSearchCtrl.setValue('another');
      tick(300);
      expect(component.filteredGcpProjects.value).toEqual(['another-project']);
    }));

    it('should filter regions based on search input', fakeAsync(() => {
        component.regionSearchCtrl.setValue('us-');
        tick(300);
        expect(component.filteredGcpRegions.value).toEqual(['us-central1', 'us-east1']);
    ...
    it('should update installer state when form value changes', () => {
        const updateStateSpy = spyOn(installerStateService, 'updateGcpConfiguration');
  ...
  describe('Navigation', () => {
    ...
    it('should emit nextStep and navigate on "Next" if the form is valid', () => {
        ...
        expect(navigateSpy).toHaveBeenCalledWith(['installer', 'deploy-infra']);
    });
  ...
  describe('Lifecycle Hooks', () => {
    it('should complete the unsubscribe subject on ngOnDestroy', () => {
        ...
        const unsubscribeCompleteSpy = spyOn(component['unsubscribe$'], 'complete');
